...
function createComment(url, request) {
  //Get comment information from message request
  const requestComment = request.body && request.body.comment;
  const response = {};
  //Check to verify request message has all the information needed to create comment
  if (requestComment && requestComment.body && requestComment.articleId && database.articles[requestComment.articleId] && requestComment.username && database.users[requestComment.username]){
    ...
    //Add comment object to the database comments array. Uses Comment id as the position in the array.
    database.comments[comment.id] = comment;
    //Associate comment with article.
    database.articles[requestComment.articleId].commentIds.push(comment.id);
    //Push the comment per the user that created it into the database.
    database.users[requestComment.username].commentIds.push(comment.id);
    //Create response for the browser to state that comment has been created.
    response.body = {comment: comment};
    //Send "CREATED" response back the browser
    response.status = 201;
  }
  else{ response.status = 400;} //Response does not contain enough information to create comment
...
function updateComment(url, request) {
  const id = Number(url.split('/').filter(segment => segment)[1]);
  //Access comment from the database using id
  const savedComment = database.comments[id];
  //Get updated comment message body from request
  const requestComment = request.body && request.body.comment;
  //Create local response object
  const response = {};
  //Check to verify if ID and request message was passed
  if (!id || !requestComment) {
    response.status = 400;
  //If no saved Comment Error
  } else if (!savedComment) {
    response.status = 404;
  } else {
    //Update the comment body with new request message
    savedComment.body = requestComment.body || savedComment.body;
    //Send response back to browser
    response.body = {comment: savedComment};
    //Send "CREATED" Post back to browswer
    response.status = 200;
  }
...
function deleteComment(url, request) {
  //Parse out ID information from URL
  const id = Number(url.split('/').filter(segment => segment)[1]);
  //Access comment from the database using id
  const savedComment = database.comments[id];
  const response = {};
  ...
  if (savedComment) {
    //Set the comment in the array equal to NULL. Fancy talk for deletion.
    database.comments[id] = null;
    //Delete comment from users database
    const userCommentIds = database.users[savedComment.username].commentIds;
    userCommentIds.splice(userCommentIds.indexOf(id), 1);
    //Delete comment from article database
...
function upvoteComment(url, request) {
  //Parse ID from URL
  const id = Number(url.split('/').filter(segment => segment)[1]);
  //Verify request message is not null. If not null get username from request message.
  const username = request.body && request.body.username;
  //Get saved comment from comments database using ID.
  let savedComment = database.comments[id];
  const response = {};

  //If the Comment exists and user name exists upvote the Comment
  if (savedComment && database.users[username]) {
    //Call the upvote function provided.
    savedComment = upvote(savedComment, username);
    //Send the response message to the browser
    response.body = {comment: savedComment};
    //Send the "CREATED" status code to browser
    response.status = 200;
  } else {
    response.status = 400;
  }
...
function downvoteComment(url, request) {
  ...
  let savedComment = database.comments[id];
  const response = {};
  //If the Comment exists and user name exists down vote the Comment
  if (savedComment && database.users[username]) {
    //Call the downvote function provided.
